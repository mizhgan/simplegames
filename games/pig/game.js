...
(() => {
  ...
  const duel = SG.duel({
    ...
    ai,
    aiDelay: 650,
    sound: (s, m) => (m.h ? 'coin' : s.bust !== null && s.bust !== undefined ? 'error' : 'drop'),
    render(s, v) {
      ...
      $('pg-die').classList.remove('roll');
      void $('pg-die').offsetWidth;
      if (L && L.d) $('pg-die').classList.add('roll');
      $('pg-pot').textContent = s.pot ? 'В ходе: ' + s.pot + ' (' + s.rolls.join(' + ') + ')' : L && L.d === 1 ? name(L.side) + ': единица — всё сгорело!' : L && L.held ? name(L.side) + ' записывает ' + L.held : '';
